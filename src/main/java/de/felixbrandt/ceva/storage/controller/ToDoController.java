...
public class ToDoController implements Controller
{
  ...
  public ToDoController(final Controller sub_controller, final Storage done_storage,
  ...
  public final Object run (final Executable executable, final DataSource source)
  {
    ...
    int todo_repeat = executable.getRepeat()
            - storage.count(executable, source, version_provider.getVersion(executable));
    ...
    if (!solution_exists || solution_pending) {
      ...
      LOGGER.debug("skipping {} for {} because result is already present",
              executable.getName(), source.getName());
    }

    return null;
  }

  public final Object runAll (final Executable executable, final DataSource source,
          final int todo_repeat)
  {
