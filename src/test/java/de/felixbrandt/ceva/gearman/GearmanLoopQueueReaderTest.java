...
public class GearmanLoopQueueReaderTest
{


  List<Future<GearmanJobResult>> result_queue;
  GearmanLoopQueueReader<Object> queue;

  @Before
  public void setUp () throws Exception
  {
    result_queue = new ArrayList<Future<GearmanJobResult>>();
    queue = new GearmanLoopQueueReader<Object>(result_queue);
  }
  ...
  @Test
  public void testGetFirstDone () throws IOException
  {
    final Future<GearmanJobResult> unfinished_job = new MockGearmanJob(null);
    result_queue.add(unfinished_job);

    final Object result_object = new Integer(42);
    final Future<GearmanJobResult> done_job = createDoneJob(result_object);
    result_queue.add(done_job);
    
    assertEquals(result_object, queue.getNext());
    assertEquals(1, result_queue.size());
  }
}
